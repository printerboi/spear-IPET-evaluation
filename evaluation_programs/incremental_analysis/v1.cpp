...
int main() {
    ...
    // LOCAL CHANGE INTRODUCTION
    unsigned int exchangeStatus = 0;

    if (clientExchangeResult == 0) {
        exchangeStatus++;
    }

    if (serverExchangeResult == 0) {
        exchangeStatus++;
    }

    std::cout << "Exchange status: " << exchangeStatus << std::endl;
